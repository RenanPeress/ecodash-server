#!/usr/bin/env python3
# ecodash-collector — gerado automaticamente pelo EcoDash
# Autenticado para: {{USER_EMAIL}}
# Gerado em:        {{GENERATED_AT}}
#
# AVISO: Não compartilhe este arquivo — contém seu token pessoal.
# Para revogar: acesse EcoDash > Meu Coletor > Regenerar Token
#
# Uso:  python ecodash-collector.py <comando> [args...]
# Ex:   python ecodash-collector.py python meu_app.py
#       python ecodash-collector.py node server.js
#
# Requisitos: pip install psutil requests
...
try:
    ...
    print(f"Erro: {e}\nInstale as dependências: pip install psutil requests")
    sys.exit(1)

# ── Configuração (gerada pelo servidor) ───────────────────────────────────────
ECODASH_TOKEN = "{{TOKEN}}"
ECODASH_API_URL = "{{API_URL}}"
...
def measure_sci(command: List[str], region: str = "BR", functional_unit: float = 1.0,
                tdp: Optional[float] = None, hardware_type: Optional[str] = None,
    ...
    result = calculate_sci(metrics, hw, region, functional_unit)
...
def submit(result: dict) -> bool:
    try:
        ...
        r.raise_for_status()
        data = r.json()
        print(f"✓ Análise #{data['id']} registrada — Grade: {data['grade']} | SCI: {data['sci_score']:.6f} gCO2eq")
        print(f"  Ver em: {ECODASH_API_URL}/analise/{data['id']}")
        return True
    except Exception as e:
        print(f"✗ Falha ao enviar para EcoDash: {e}")
        fallback = "ecodash-result.json"
        with open(fallback, "w") as f:
            json.dump(result, f, indent=2, ensure_ascii=False)
        print(f"  Resultado salvo localmente em: {fallback}")
        return False


def main():
    if len(sys.argv) < 2:
        print("Uso: python ecodash-collector.py <comando> [args...]")
        print("Ex:  python ecodash-collector.py python meu_app.py")
        sys.exit(1)

    command = sys.argv[1:]
    region = "BR"

    print(f"EcoDash Collector")
    print(f"Medindo: {' '.join(command)}")
    print("─" * 50)

    result = measure_sci(command, region=region)

    print(f"Duração:      {result['metrics']['duration_seconds']:.2f}s")
    print(f"CPU (média):  {result['metrics']['cpu_percent_avg']:.1f}%")
    print(f"RAM (média):  {result['metrics']['memory_used_mb_avg']:.1f} MB")
    print(f"Energia:      {result['energy_kwh']:.8f} kWh")
    print(f"SCI Score:    {result['sci_score']:.6f} gCO2eq")
